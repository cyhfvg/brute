...
| `execute` | 认证成功后的命令. 仅 `ssh`/`ftp`/`mysql`/`postgresql`/`redis`/`oracle`/`winrm`/`zookeeper`/`memcached`/`mongodb`/`elasticsearch`/`docker`/`snmp`/`activemq`/`rabbitmq`/`mssql`/`kafka`/`kibana`/`nfs`/`telnet`/`ldap`/`grafana`/`prometheus` | 无 |
