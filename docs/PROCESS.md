...
- HTTP 系（`http` / `tomcat` / `winrm` / VNC web Basic / `elasticsearch` / `docker` / `kibana` / `grafana` / `prometheus` / `jenkins` / `couchdb` / `clickhouse` / `neo4j` / `etcd` / `influxdb` / `solr` / `minio` / `nacos` / `nexus` / `jboss` / `druid` / `spark` / `hadoop` / `kubelet` / `gitlab`）：`reqwest::Proxy`（`reqwest` 启用 `socks` feature）；`winrm-rs` 使用 `WinrmConfig.proxy`
