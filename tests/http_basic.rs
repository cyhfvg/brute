//! HTTP Basic Auth public API and end-to-end attempt tests.
...
use brute::protocol::http::{
    ...
    normalize_path, scheme_skips_cert_verification,
};
use brute::protocol::{AttemptContext, AttemptOutcome, BruteModule};
...
fn builds_client_for_http_and_https_schemes() {
    let http_client = build_http_basic_client(Duration::from_secs(2), HttpUrlScheme::Http, None)
        .expect("http client");
