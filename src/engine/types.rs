...
pub(crate) const ALL_PROTOCOLS: [Protocol; 43] = [
    Protocol::Ssh,
    Protocol::Ftp,
    Protocol::Mysql,
    Protocol::Postgresql,
...
pub fn parse_protocol(name: &str) -> Result<Protocol> {
    match name.trim().to_ascii_lowercase().as_str() {
        ...
        "neo4j" => Ok(Protocol::Neo4j),
        "etcd" => Ok(Protocol::Etcd),
        "influxdb" | "influx" => Ok(Protocol::Influxdb),
        "solr" => Ok(Protocol::Solr),
        "minio" => Ok(Protocol::Minio),
        "nacos" => Ok(Protocol::Nacos),
        "nexus" => Ok(Protocol::Nexus),
        "jboss" | "wildfly" => Ok(Protocol::Jboss),
        "druid" => Ok(Protocol::Druid),
        "spark" => Ok(Protocol::Spark),
        "hadoop" | "hdfs" => Ok(Protocol::Hadoop),
        other => bail!(
