...
async fn attempt_once(ctx: &AttemptContext) -> Result<AttemptSuccess, HadoopAttemptError> {
    ...
    let url = api_url(ctx.url_scheme, &ctx.target_host, port, "/jmx");
...
///
/// assert_eq!(api_url(brute::cli::HttpUrlScheme::Http, "10.0.0.5", 9870, "/jmx"), "http://10.0.0.5:9870/jmx");
/// ```
...
async fn probe_jmx(ctx: &TargetContext) -> Option<String> {
    ...
    let url = api_url(ctx.url_scheme, &ctx.target_host, ctx.port(), "/jmx");
