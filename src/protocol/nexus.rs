//! Nexus Repository Manager login, unauthorized probe, and post-auth HTTP API (`-x`).
//!
//! Empty credentials probe `GET /service/rest/v1/repositories` without Authorization.
...
impl NexusModule {
    /// Creates a new Nexus module instance.
    ///
    /// # Parameters
    ///
    ...
    ///
    /// ```
    /// use brute::protocol::nexus::NexusModule;
    ///
    /// let _module = NexusModule::new(5000);
    /// ```
...
#[async_trait]
impl BruteModule for NexusModule {
...
async fn attempt_once(ctx: &AttemptContext) -> Result<AttemptSuccess, NexusAttemptError> {
    ...
    let url = http_attempt_url(ctx, "/service/rest/v1/security/users");
...
async fn execute_command(
    ...
    let response = request
        .send()
        .await
        .map_err(|err| NexusAttemptError::Command(err.to_string()))?;
    ...
    if status.is_success() {
        ...
        Err(NexusAttemptError::Command(format!(
            "HTTP {status}: {}",
            trim_body(&body)
        )))
    }
...
/// # Parameters
///
/// - `command`: CLI `-x` value (`repos`, `status`, or a path).
///
/// # Returns
///
/// Absolute path.
...
///
/// ```
/// use brute::protocol::nexus::execute_path;
///
/// assert_eq!(execute_path("repos"), "/service/rest/v1/repositories");
/// assert_eq!(execute_path("status"), "/service/rest/v1/status");
/// ```
pub fn execute_path(command: &str) -> String {
    let trimmed = command.trim();
    match trimmed.to_ascii_lowercase().as_str() {
        "repos" | "repositories" => "/service/rest/v1/repositories".to_string(),
        "status" => "/service/rest/v1/status".to_string(),
        "blobs" | "blobstores" => "/service/rest/v1/blobstores".to_string(),
...
async fn probe_status(ctx: &TargetContext) -> Option<String> {
    ...
    let url = http_target_url(ctx, "/service/rest/v1/status");
