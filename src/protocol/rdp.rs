...
impl BruteModule for RdpModule {
    ...
    async fn attempt(&self, ctx: &AttemptContext) -> AttemptOutcome {
        ...
        run_blocking_with_timeout(timeout, move || {
            try_rdp_login(&host, port, &username, &password, timeout, proxy.as_ref())
        })
        .await
    }
}
...
fn probe_rdp_port(
    ...
    match connect_rdp_stream(host, port, timeout, proxy) {
        Ok(_stream) => Some(format!("rdp port open on {host}:{port}")),
        Err(_) => None,
    }
}

/// Opens a TCP stream to the RDP target, optionally through an outbound proxy.
fn connect_rdp_stream(
    host: &str,
    port: u16,
    timeout: Duration,
    proxy: Option<&crate::proxy::ProxyConfig>,
    ...
    if let Some(proxy) = proxy {
        return crate::proxy::connect_std(proxy, host, port, timeout)
            .map_err(|err| format!("rdp transport error: {err}"));
    }

    let server_addr =
        resolve_addr(host, port).map_err(|err| format!("rdp resolve error: {err}"))?;
    TcpStream::connect_timeout(&server_addr, timeout)
        .map_err(|err| format!("rdp transport error: {err}"))
}
...
#[cfg(test)]
mod tests {
    use super::looks_like_auth_failure_message;
