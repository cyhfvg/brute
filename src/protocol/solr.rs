...
type SolrAttemptError = crate::protocol::http_attempt::HttpAttemptFailure;
...
async fn attempt_once(ctx: &AttemptContext) -> Result<AttemptSuccess, SolrAttemptError> {
    ...
    let url = http_attempt_url(ctx, "/solr/admin/info/system");
...
async fn probe_system(ctx: &TargetContext) -> Option<String> {
    ...
    let url = http_target_url(ctx, "/solr/admin/info/system");
